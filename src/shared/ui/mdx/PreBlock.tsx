...
import { CodeBlockClient } from "@/shared/ui/mdx/CodeBlockClient";
...
export async function PreBlock(props: React.HTMLAttributes<HTMLPreElement>) {
  ...
  if (isValidElement(children)) {
    ...
    if (
      props &&
      (props.className || typeof props.children === "string" || Array.isArray(props.children))
    ) {
      codeProps = props;
    }
  }
  ...
  const innerHtml = html.replace(/^<pre[^>]*><code[^>]*>/, "").replace(/<\/code><\/pre>$/, "");

  return <CodeBlockClient html={innerHtml} language={language} filename={filename} />;
}
